...
❗ The `*` character does not match hidden files. To match hidden files:
...
Brace epansion `{...}` indicates that the contents within the braces are expanded into all the values separated by commas
